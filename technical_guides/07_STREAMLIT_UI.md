# Technical Guide 07: Streamlit Chat UI

## 📋 Overview
This guide covers building the **Streamlit chat interface** — a polished, interactive UI where users can upload documents and have a multi-turn conversation with the QA Bot, complete with source citations.

---

## Step 1: App Entry Point (`app.py`)
...
with st.sidebar:
    ...
    llm_provider = st.selectbox("LLM Provider", ["openai", "huggingface"])
...
## Step 3: UI Features Summary

| Feature | Description |
|---------|-------------|
| **Multi-file Upload** | Upload PDF, DOCX, TXT via sidebar. |
| **Processing Status** | Spinner + success message after ingestion. |
| **Chat Interface** | Native Streamlit `st.chat_message` bubbles. |
| **Source Citations** | Expandable "📄 View Sources" below each answer. |
| **Clear Session** | Reset all documents and chat history. |
| **Top-K Slider** | Adjust number of retrieved chunks. |
| **LLM Selector** | Switch between OpenAI and HuggingFace. |

---

## Step 4: Sample Questions to Test

After uploading a company policy PDF, try:
```
1. "What is the annual leave policy?"
2. "How many sick days are allowed?"
3. "Can I carry over unused leave to next year?"
4. "What is the process to apply for maternity leave?"
5. "Who do I contact for HR queries?"
```
...
- [ ] File upload and processing works for PDF, DOCX, TXT.
- [ ] Chat messages display correctly with user/assistant bubbles.
- [ ] Source citations expand correctly below each answer.
- [ ] Clear button resets all state.
- [ ] Multi-turn follow-up questions work correctly.
